"""
spheres.land — Portfolio backend
Serves completed SPHERES productions from talent-agent production files.
"""
...
app = FastAPI(title="spheres.land")
...
def _load_productions():
    """Scan production_files for completed SPHERES productions."""
    productions = []
    ...
    for project_dir in sorted(os.listdir(PRODUCTION_DIR)):
        ...
        for data_file in glob.glob(os.path.join(full_path, "*_Data.json")):
            try:
                ...
                if project.get("game_type") != "spheres":
                    continue
...
def list_productions(sort: str = "score"):
    """List all completed SPHERES productions."""
    ...
    for p in prods:
        ...
        items.append({
            ...
            "city": parcel.get("city", "Philadelphia"),
            ...
            "chron_total": scores.get("total", 0),
    ...
    if sort == "score":
        items.sort(key=lambda x: x["chron_total"], reverse=True)
    elif sort == "title":
        items.sort(key=lambda x: x["title"])
    elif sort == "ip":
        items.sort(key=lambda x: x["ip_count"], reverse=True)
    elif sort == "neighborhood":
        items.sort(key=lambda x: x["neighborhood"] or "")
...
def get_production(project_id: str):
    """Get a complete SPHERES production."""
    prods = _load_productions()
    for p in prods:
